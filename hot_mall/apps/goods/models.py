...
class Brand(BaseModel):
    ...
    logo = models.ImageField(verbose_name='Logo图片', null=True, blank=True)
...
class SPU(BaseModel):
    ...
    category2 = models.ForeignKey(GoodsCategory, on_delete=models.PROTECT, related_name='cat2_spu', verbose_name='二级类别', null=True, blank=True)
    category3 = models.ForeignKey(GoodsCategory, on_delete=models.PROTECT, related_name='cat3_spu', verbose_name='三级类别', null=True, blank=True)
...
class SKU(BaseModel):
    ...
    default_image = models.ImageField(max_length=200, default='', null=True, blank=True, upload_to='goods/', verbose_name='默认图片')
    production_date = models.DateField(null=True, blank=True, verbose_name='生产日期')
    shelf_life = models.IntegerField(default=0, verbose_name='保质期(天)', help_text='请输入保质期，单位为天')
    expiration_date = models.DateField(null=True, blank=True, verbose_name='过期日期', help_text='商品的过期日期')
    barcode = models.CharField(
        max_length=30,  # 兼容EAN-13、EAN-8、UPC-A等常见条码类型，预留足够长度
        unique=False,
        verbose_name='商品条码',
        blank=True,  # 可选：允许后台录入时暂时不填（根据业务需求决定）
        null=True,  # 可选：允许数据库中该字段为NULL（与blank配合使用）
        help_text='请输入商品标准条码（如EAN-13、UPC-A格式）'
    )
    near_expiry_num = models.IntegerField(default=0, verbose_name='临保质期数量', help_text='临保质期商品数量')
    expired_num = models.IntegerField(default=0, verbose_name='过期商品数量', help_text='过期商品数量')

    class Meta:
        db_table = 'tb_sku'
        verbose_name = '商品SKU'
        verbose_name_plural = verbose_name# 新增索引，提升查询效率
        indexes = [
            models.Index(fields=['barcode'], name='idx_sku_barcode'),
            models.Index(fields=['category'], name='idx_sku_category'),
            models.Index(fields=['name'], name='idx_sku_name'),
            models.Index(fields=['is_launched'], name='idx_sku_launched'),
        ]
